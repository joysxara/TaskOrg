...
layout = [
            ...
            [sg.Text('Task Name: ', size=(18,1)), sg.InputText(do_not_clear=False)],
            #do_not_clear=False clears the input box everytime event is True
            [sg.Text('Priority: ', size=(18,1)), sg.InputText(do_not_clear=False)],
            [sg.Text('Due Date (YYYYMMDD): ', size=(18,1)), sg.InputText(do_not_clear=False)],
...
while True:
    ...
    if event is None:
        ...
        root.insert(values[0], int(values[1]), int(values[2]))
